...
from Set_parameters import cn_high_mad_cutoff, cn_low_mad_cutoff, c_low_mad_cutoff, c_high_mad_cutoff, n_low_mad_cutoff, n_high_mad_cutoff
...
from Set_parameters import system_correction
...
output_path = f"{source_path}/output_abs_sys_corr"
...
for rc in rc_list:
    ...
    def df_pmap_merge(dataframe, p_map):
        ...
        dataframe_merge.dropna(axis=0, subset=["cell__treatment"], inplace=True)
    ...
    def true_signal(dataframe):
        ...
        plt.close()
        dataframe2 = dataframe2.loc[(dataframe2["cyto_mean_log10"] >= c_median_low) & (dataframe2["cyto_mean_log10"] <= c_median_high) & (dataframe2["nuc_mean_log10"] >= n_median_low) & (dataframe2["nuc_mean_log10"] <= n_median_high)]
        ...
        plt.close()
        dataframe2 = dataframe2[(dataframe2["cn_ratio_log10"] >= cn_median_low) & (dataframe2["cn_ratio_log10"] <= cn_median_high)]
    ...
    def df_to_hmap_nuc_only(dataframe, values, data_identity):
        ...
        if data_identity == "reporter":
            ...
            dataframe_matrix[["cell", "treatment"]] = dataframe_matrix["cell__treatment"].str.split(pat="__", n=1,
                                                                                                    expand=True)
            dataframe_matrix[["mutation", "drug"]] = dataframe_matrix["treatment"].str.split(pat="_", n=1, expand=True)
            ###########################################################################################
            # correct using all untreated t0 normalized
            ############################################################################################
            mutation_n = dataframe_matrix["mutation"].nunique()
            if mutation_n == 1:
                rolling_window = 2
            else:
                rolling_window = 1
            df_correct = dataframe_matrix.loc[dataframe_matrix['drug'] == system_correction].copy()
            dataframe_matrix.drop(["site", "cell__treatment", "cell", "treatment", "mutation", "drug"], axis=1,
                                  inplace=True)
            df_correct.drop(["site", "cell__treatment", "cell", "treatment", "mutation", "drug"], axis=1, inplace=True)
            df_correct2 = df_correct.mean(axis=0).rolling(rolling_window, min_periods=1).mean()
            dataframe_matrix = dataframe_matrix.subtract(df_correct2, axis=1)
    ...
    def reporter_lineplot(channel):
        ...
        treatment_n = df_melt["treatment"].nunique()
        if treatment_n > heatmap_col:
            col_n = heatmap_col
        else:
            col_n = treatment_n
        ax = sns.relplot(data=df_melt, kind="line", x="time_min", y="activity",
                         col="treatment", col_wrap=col_n, legend=None, ci=None, markers=True, color=color)
